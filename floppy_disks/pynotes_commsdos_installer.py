...
def fxn():
    warnings.warn("Syntax warning found.", SyntaxWarning)
...
print("\n")
sys_load(5, "Preparing space to install pynotes... ")
with open("dos_storage_dir\\pynotes.py", "w") as f:
    f.write("""
import datetime
def changelog():
...
with open("dos_storage_dir\\pynotes_ota_upd.txt", "w") as f:
    f.write("1.5.1a 4.1KB")
    f.close()

for remaining in range(-1, 101, 1):
    sys.stdout.write("\r>Installing pynotes... [{:.2f}%] ".format(remaining))
    sys.stdout.flush()
    time.sleep(0.06)

time.sleep(0.9)
print("\n>'pynotes.py' has been installed into your dos_storage_dir!")
